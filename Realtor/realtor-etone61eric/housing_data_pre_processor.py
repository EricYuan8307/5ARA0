...
def split_housing_train_test(data):
    ...
    stratify = pd.cut(data["median_income"], bins, labels=[1, 2, 3, 4, 5])  # <COMPLETE THE OPTIONS>

    return train_test_split(data, random_state=42)  # <COMPLETE THE OPTIONS>
# </ASSIGNMENT 3.1>
...
def pre_process_housing_data(train_set, test_set, write_to_file=True):
    ...
    num_pipeline = make_pipeline(SimpleImputer(strategy='median'),HousingAttributesExtender(True),StandardScaler())  # <COMPLETE ARGUMENTS>
    # Use the median available values to replace the missing value
    # default missing value np.nan, strategy=median
    ...
    # The default missing value np.nan, strategy=constant with missing filling
